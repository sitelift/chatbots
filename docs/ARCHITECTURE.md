...
**Website import** (`POST /api/admin/import`): SSRF-safe crawl — DNS + private-range blocking, redirect caps, same-origin links only (up to 5 pages fetched **in parallel**, ~30k chars combined, junk-path denylist) → HTML-to-text extraction → the configured AI provider (the chatbot's model or the Settings default) reads the combined page text in **one call** (short extract prompt, `reasoning: { effort: "none" }` for fast non-thinking output, no max-tokens cap) and fills the `businessFactsSchema` JSON, with one retry on invalid/empty JSON. Empty or garbage extractions fail with `EXTRACTION_FAILED` instead of returning a blank draft. An optional **provider pin** setting (OpenRouter only) routes every request to one specific upstream (`provider: { only: [slug], allow_fallbacks: false }`) for consistent speed. The dashboard presents what was read and applies it as an editable draft — import never overwrites until the owner saves.
